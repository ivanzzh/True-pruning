...
def state_dict_compress1(state_dict):
    ...
    for key in state_dict:
        ...
        if key[-4:] != 'bias':
            ...
            non_zero_num = torch.sum(non_zero_index).item()
            output_channel = state_dict[key].shape[0]
            filter_length = torch.zeros(output_channel, dtype=torch.int32)
            start_point = torch.zeros(output_channel, dtype=torch.int32)
...
def transfer(row_size, col_size, kernel_size, stride):
    row_size2 = int((row_size - kernel_size) / stride + 1)
